...
use crate::cli::{AuthCommand, AuthAction};
...
async fn login() -> Result<()> {
    ...
    let providers = vec![
        ("Anthropic Claude", "Use Claude Pro/Max subscription or API key"),
        ("GitHub Copilot", "Use GitHub Copilot subscription (recommended)"),
        ...
        ("Perplexity", "Cost-effective AI models with web search capabilities"),
    ...
    let selection = Select::with_theme(&ColorfulTheme::default())
        ...
        .items(&providers.iter().map(|(name, desc)| format!("{:<20} {}", name, desc.dimmed())).collect::<Vec<_>>())
        .default(0)
        .interact()?;
...
async fn handle_claude_oauth() -> Result<()> {
    ...
    println!("{}", "This will use your Claude Pro/Max subscription".dimmed());
...
async fn handle_claude_api_key_creation() -> Result<()> {
    ...
    println!("{}", "This will create a new API key in your Claude Console".dimmed());
    
    // For now, redirect to manual entry - API key creation requires additional OAuth flow
    println!("{}", "⚠️  Automatic API key creation not yet implemented".yellow());
    println!("{}", "Please create an API key manually at: https://console.anthropic.com/settings/keys".cyan());
...
async fn handle_openai_auth() -> Result<()> {
    ...
    println!("{}", "Get your API key from: https://platform.openai.com/api-keys".cyan());
...
async fn handle_ollama_auth() -> Result<()> {
    ...
    if use_local {
        ...
        println!("{}", "✓ Configured for local Ollama (http://localhost:11434)".green());
    } else {
        ...
        println!("{}", format!("✓ Configured for remote Ollama ({})", url).green());
    }
    ...
    println!("{}", format!("✓ Ollama configured with model: {}", model).green().bold());
    Ok(())
}
...
async fn handle_gemini_auth() -> Result<()> {
    ...
    println!("{}", "Get your API key from: https://aistudio.google.com/app/apikey".cyan());
...
async fn handle_azure_auth() -> Result<()> {
    ...
    println!("{}", "✓ Azure OpenAI configured successfully".green().bold());
    Ok(())
}
...
async fn handle_manual_api_key(provider: &str) -> Result<()> {
    ...
    println!("{}", format!("✓ {} API key configured successfully", provider).green().bold());
    Ok(())
}
...
async fn handle_github_copilot_auth() -> Result<()> {
    ...
    println!("{}", "GitHub Copilot provides free AI assistance to subscribers".dimmed());
    
    // TODO: Implement GitHub OAuth device flow
    println!("{}", "⚠️  GitHub OAuth device flow not yet implemented".yellow());
    ...
    println!("{}", "✓ GitHub Copilot configured (requires GitHub CLI auth)".green().bold());
    Ok(())
}
...
async fn handle_openrouter_auth() -> Result<()> {
    ...
    println!("{}", "Get your API key from: https://openrouter.ai/keys".cyan());
...
async fn handle_groq_auth() -> Result<()> {
    ...
    println!("{}", "Get your API key from: https://console.groq.com/keys".cyan());
...
async fn handle_deepseek_auth() -> Result<()> {
    ...
    println!("{}", "Get your API key from: https://platform.deepseek.com/api_keys".cyan());
...
async fn handle_mistral_auth() -> Result<()> {
    ...
    println!("{}", "Get your API key from: https://console.mistral.ai/".cyan());
...
async fn handle_aws_bedrock_auth() -> Result<()> {
    ...
    println!("{}", "AWS Bedrock supports multiple authentication methods".dimmed());
    ...
    match method {
        0 => {
            println!("{}", "Enter your AWS Bedrock API key (new feature in 2025)".cyan());
            println!("{}", "This will be stored in AWS_BEARER_TOKEN_BEDROCK".dimmed());
        ...
        1 => {
            ...
            println!("{}", format!("✓ AWS Bedrock configured with profile: {}", profile).green().bold());
            config.save()?;
            Ok(())
        }
        2 => {
            ...
            println!("{}", "✓ AWS Bedrock configured for environment variables".green().bold());
            config.save()?;
            Ok(())
        }
        3 => {
...
async fn handle_together_auth() -> Result<()> {
    ...
    println!("{}", "Get your API key from: https://api.together.xyz/settings/api-keys".cyan());
...
async fn handle_deepinfra_auth() -> Result<()> {
    ...
    println!("{}", "Get your API key from: https://deepinfra.com/dash/api_keys".cyan());
...
async fn handle_huggingface_auth() -> Result<()> {
    ...
    println!("{}", "Get your API key from: https://huggingface.co/settings/tokens".cyan());
...
async fn handle_github_models_auth() -> Result<()> {
    ...
    println!("{}", "Get your token from: https://github.com/settings/personal-access-tokens".cyan());
...
async fn handle_manual_api_key_with_env(provider: &str, env_var: &str) -> Result<()> {
    ...
    println!("{}", format!("✓ {} configured with environment variable {}", provider, env_var).green().bold());
    println!("{}", format!("  Environment variable {} has been set for this session", env_var).dimmed());
    Ok(())
}
...
async fn status() -> Result<()> {
    ...
    if config.api_key.is_some() {
        ...
        println!("  Provider: {}", config.ai_provider.as_deref().unwrap_or("openai"));
        return Ok(());
    }
            ...
            if let Some(refresh_token) = &tokens.refresh_token {
                ...
                match oauth_client.refresh_token(refresh_token).await {
                    Ok(token_response) => {
                        ...
                        token_storage::store_tokens(
                            &token_response.access_token,
                            token_response.refresh_token.as_deref().or(Some(refresh_token)),
                            token_response.expires_in,
                        )?;
                    ...
                    Err(e) => {
                        tracing::warn!("Failed to refresh token: {}. User may need to re-authenticate.", e);
                    }
                }
            }
        }
...
async fn handle_perplexity_auth() -> Result<()> {
    ...
    println!("{}", "Get your API key from: https://www.perplexity.ai/settings/api".cyan());
