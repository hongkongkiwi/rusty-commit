...
fn test_openrouter_provider_config() {
    ...
    assert_eq!(loaded.api_url.as_deref(), Some("https://openrouter.ai/api/v1"));
...
fn test_groq_provider_config() {
    ...
    assert_eq!(loaded.api_url.as_deref(), Some("https://api.groq.com/openai/v1"));
...
fn test_aws_bedrock_provider_config() {
    ...
    assert_eq!(loaded.model.as_deref(), Some("us.anthropic.claude-3-5-haiku-20241022-v1:0"));
    
    // Verify environment variable was set
    assert_eq!(std::env::var("AWS_BEARER_TOKEN_BEDROCK").unwrap(), "test_bedrock_token");
...
fn test_azure_openai_provider_config() {
    ...
    assert_eq!(loaded.api_url.as_deref(), Some("https://test-resource.openai.azure.com"));
...
fn test_together_ai_provider_config() {
    ...
    assert_eq!(loaded.api_url.as_deref(), Some("https://api.together.xyz/v1"));
    assert_eq!(loaded.model.as_deref(), Some("meta-llama/Llama-3.2-3B-Instruct-Turbo"));
}
...
fn test_deepinfra_provider_config() {
    ...
    assert_eq!(loaded.api_url.as_deref(), Some("https://api.deepinfra.com/v1/openai"));
    assert_eq!(loaded.model.as_deref(), Some("meta-llama/Llama-3.2-3B-Instruct"));
}
...
fn test_huggingface_provider_config() {
    ...
    assert_eq!(loaded.api_url.as_deref(), Some("https://api-inference.huggingface.co/v1"));
    assert_eq!(loaded.model.as_deref(), Some("meta-llama/Llama-3.2-3B-Instruct"));
}
...
fn test_github_models_provider_config() {
    ...
    assert_eq!(loaded.api_url.as_deref(), Some("https://models.inference.ai.azure.com"));
...
fn test_gemini_provider_config() {
    ...
    assert_eq!(loaded.api_url.as_deref(), Some("https://generativelanguage.googleapis.com/v1beta"));
...
fn test_ollama_provider_config() {
    ...
    assert_eq!(loaded_remote.api_url.as_deref(), Some("http://192.168.1.100:11434"));
}
...
fn test_custom_provider_config() {
    ...
    assert_eq!(loaded.api_url.as_deref(), Some("https://api.custom-provider.com/v1"));
...
fn test_all_supported_providers_list() {
    ...
    for provider in supported_providers {
        ...
        assert!(config.save().is_ok(), "Failed to save config for provider: {}", provider);
...
fn test_model_defaults_for_providers() {
    ...
    for (provider, expected_model) in provider_model_mappings {
        ...
        assert!(!expected_model.is_empty(), "Provider {} should have a default model", provider);
        
        // Models should be in expected format
        if provider == "openrouter" {
            assert!(expected_model.contains("/"), "OpenRouter models should include provider prefix");
        }
        
        if provider == "amazon-bedrock" {
            assert!(expected_model.starts_with("us."), "Bedrock models should include region prefix");
        }
        
        if provider == "together" || provider == "deepinfra" || provider == "huggingface" {
            assert!(expected_model.contains("/"), "Hosted model providers should include model path");
        }
    }
}